...
class Encriptografador(QtGui.QMainWindow):
    ...
    def __init__(self, width, height, key):
        ...
        self.e = E()
        self.e.updateEText.connect(self.updateETextFn)
        self.e.updateDText.connect(self.updateDTextFn)

        self.show()

    def updateETextFn(self, txt):
        self.encrypted_textbox.setPlainText(txt)

    def updateDTextFn(self, txt):
        self.decrypted_textbox.setPlainText(txt)
    ...
    def cryptbtn_click(self):
        ...
        if self.mode == "e":            
            ...
            self.e.encrypt(self.decrypted_textbox.toPlainText(), self.progress_bar, self.key)
        elif self.mode == "d":
...
class E(QtCore.QObject):

    updateEText = QtCore.pyqtSignal(str)
    updateDText = QtCore.pyqtSignal(str)

    def encrypt(self, msg, progress_bar, key):
        t = Thread(target=self.encrypt_message, args=(msg, progress_bar, key))
        t.daemon = True
        t.start()

    def encrypt_message(self, msg, progress_bar, key):
        ...
        for char in msg:
            encrypted_char = ord(char) * ord(key[counter])
        ...
        print("Finished encrypting")
        self.updateEText.emit(encrypted_msg)

    def decrypt_message(self, encrypted_msg, progress_bar, key):
        ...
        for encrypted_element in encrypted_list:
            encrypted_char = int(encrypted_element)
            decryption_char = ord(key[counter])
            decrypted_int = int(encrypted_char / decryption_char)
        ...
        self.updateDText.emit(decrypted_msg)
